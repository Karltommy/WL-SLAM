...
void addOdomFactor()
{
    ...
    else
    {
        ...
        gtsam::noiseModel::Diagonal::shared_ptr odometryNoise = gtsam::noiseModel::Diagonal::Variances((gtsam::Vector(6) << 1e-6, 1e-6, 1e-6, 1e-4, 1e-4, 1e-3).finished());
...
void addGravityFactor()
{
    if(vecgravity.size()<2)
       return;
    gtsam::noiseModel::Diagonal::shared_ptr gravityNoise = gtsam::noiseModel::Diagonal::Variances((gtsam::Vector(3) << 1e-12, 1e-12, 1e-12).finished());
